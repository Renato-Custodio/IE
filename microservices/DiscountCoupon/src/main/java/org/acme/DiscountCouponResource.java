        ...
        private void initdb() {
                ...
                client.query("DROP TABLE IF EXISTS DiscountCoupons").execute()
                                .flatMap(r -> client.query(
                                                "CREATE TABLE DiscountCoupons (id SERIAL PRIMARY KEY, idLoyaltyCard BIGINT UNSIGNED, idShop BIGINT UNSIGNED, discount TEXT NOT NULL, DateTime DATETIME)")
                                                .execute())
                                .flatMap(r -> client.query(
                                                "INSERT INTO DiscountCoupons (idLoyaltyCard,idShop,discount,DateTime) VALUES (1,1,'discount1','2038-01-19 03:14:07')")
                                                .execute())
                                .flatMap(r -> client.query(
                                                "INSERT INTO DiscountCoupons (idLoyaltyCard,idShop,discount,DateTime) VALUES (2,1,'discount2','2038-01-19 03:14:07')")
                                                .execute())
                                .flatMap(r -> client.query(
                                                "INSERT INTO DiscountCoupons (idLoyaltyCard,idShop,discount,DateTime) VALUES (1,3,'discount3','2038-01-19 03:14:07')")
                                                .execute())
                                .flatMap(r -> client.query(
                                                "INSERT INTO DiscountCoupons (idLoyaltyCard,idShop,discount,DateTime) VALUES (4,2,'discount4','2038-01-19 03:14:07')")
                                                .execute())
                                .await().indefinitely();
        }
