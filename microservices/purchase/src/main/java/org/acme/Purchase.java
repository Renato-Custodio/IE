...
public class Purchase 
{
    ...
    public Purchase(Long id, LocalDateTime timestamp, Float price, String product, String supplier, String shopName, Long loyaltyCardId) {
        this.id = id;
    ...
    public static Multi<Purchase> findAll(MySQLPool client) {
        return client.query("SELECT id, date_time, price, product , supplier, shop_name, loyalty_card_id FROM Purchases ORDER BY id ASC").execute()
                .onItem().transformToMulti(set -> Multi.createFrom().iterable(set))
                .onItem().transform(Purchase::from);
    }
    
    public static Uni<Purchase> findById(MySQLPool client, Long id) {
        return client.preparedQuery("SELECT id, date_time, price, product , supplier, shop_name, loyalty_card_id FROM Purchases WHERE id = ?").execute(Tuple.of(id))
                .onItem().transform(RowSet::iterator) 
                .onItem().transform(iterator -> iterator.hasNext() ? from(iterator.next()) : null); 
    }

    public static Multi<Purchase> findByLoyaltyCardId(MySQLPool client, Long loyaltyCardId) {
        return client.preparedQuery("SELECT id, date_time, price, product, supplier, shop_name, loyalty_card_id FROM Purchases WHERE loyalty_card_id = ?").execute(Tuple.of(loyaltyCardId))
            .onItem().transformToMulti(set -> Multi.createFrom().iterable(set))
            .onItem().transform(Purchase::from);
    }
